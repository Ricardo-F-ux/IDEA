 ...
 *          重写的hashCode()和equals()保持一致性，相等的对象必须具有相等的散列码
 *
 *          重写技巧：对象中用作 equals() 方法比较的 Field，都应该用来计算 hashCode 值。
 *
 * @author EnvyFL
 * @create 2021-04-14-9:21 上午
 * @class
 */
public class SetTest {
    /*
  一、Set：无序的，不可重复的数据
    1.无序的：不等于随机性。 存储的数据在底层数组中并非按照数组的索引顺序添加，而是根据数据的hash值决定的

    2.不可重复的：保证添加的元素按照equals()判断时，不能返回true，相同元素只能添加一个
  ...
  二、添加元素的过程：以HashSet为例：
    ...
    对于添加成功的情况2与情况3而言：元素a与已经存在指定索引位置上数据以链表的方式存储。
