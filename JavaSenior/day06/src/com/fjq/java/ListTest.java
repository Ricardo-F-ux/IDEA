 ...
 *                     ｜--- LinkedList ：对于频繁插入、删除操作，使用此类效率比ArrayList高；底层使用的是双向链表存储
 *                     ｜--- Vector ：作为List接口的古老实现类，线程安全，效率不高；底层使用Object[] elementData存储
 *
 * 2.ArrayList的源码分析：
 *      2.1jdk 7 的情况下
 ...
 *
 *          list.add(123);//第一次调用add()时，底层才创建长度10的数组，并将数据123添加到elementData
 *          ...
 *          后续的添加和扩容操作与jdk 7 无异
 *      2.3小结：jdk 7 中的ArrayList的创建类似于单例的饿汉式，而jdk 8 中的ArrayList的对象
 *              创建类似于单例的懒汉式，延迟了数组的创建，节省了内存
 *
 * 3.LinkedList的源码分析
